#!/usr/bin/env python3
"""
Alternative Tickers Database Manager
Query and manage the alternative tickers database
"""
...
def get_db_connection():
    ...
    if not os.path.exists(DB_PATH):
        ...
        print("Run ticker_collector.py first to create the database.")
        sys.exit(1)
...
def show_sample_tickers(count: int = 20):
    ...
    print(f"Sample of {len(tickers)} tickers from Alternative Tickers database:")
    print("-" * 80)

    for ticker in tickers:
        source = ticker['source'].replace('wikipedia_', '').replace('_sample', '')
        print("6")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Alternative Tickers Database Manager")
    parser.add_argument('action', choices=['stats', 'sample', 'search', 'sources', 'export'],
    ...
    if args.action == 'stats':
        stats = get_database_stats()
        print("Alternative Tickers Database Statistics")
        print("=" * 50)
